...
from ihp.cells import nmos, pmos_hv, guard_ring
...
def power_hv_pmos(
    ...
    pmosSep=-0.64 #Needed for both psD to meet.
    ...
    if doubleRow:
        ...
        m1 = c.add_ref(pmos_hv(width=width_double_row, length=length, nf=nf))
        m1.dymin=0
        m1.dxmin=0
        m2 = c.add_ref(pmos_hv(width=width_double_row, length=length, nf=nf))
        m2.dymin=0
        m2.dxmin=0
        ...
        m1 = c.add_ref(pmos_hv(width=width, length=length, nf=nf))
        m = [m1]
...
if __name__ == "__main__":
    top = power_hv_pmos(width=2175, length=0.45, nf=150)
