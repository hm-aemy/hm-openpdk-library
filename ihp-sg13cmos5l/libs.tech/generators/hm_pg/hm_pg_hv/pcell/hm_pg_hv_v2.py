...
from hm_pg_hv_met3_v2 import hm_pg_hv_met3
from hm_pg_hv_met4_v2 import hm_pg_hv_met4
from hm_pg_hv_lv2hv import lv2hv
...
def hm_pg_lv(
    ...
    # the pmos hv has a left offset of 1.57 and
    # a right offset of 1.57 -> this is parametrized on the power pmos layout.
    # on the future we should take the value from there
    pmosMaxWf=width-(1.57 + 1.57)
    if pmosMaxWf>10:
    ...
    pmosMaxHeight=height-(3.49+2.8+5.5+3*blockSep)-invInConnL-0.44
    ...
    pmosMaxHeighteff=pmosMaxHeight-2*1.3 #TODO: This must be changed to relative
    ...
    power_pmos_ref = c.add_ref(
        power_hv_pmos(width=pmosWidthOnGrid, length=pmosL, nf=pmosMaxnf, sd_sep=pmosSDsep)
    ).rotate(270)
    ...
    lv2hv_ref.dymin = 0
    lv2hv_ref.dxmin = 0
    lv2hv_ref.dmovey(inverter_ref.ymax+blockSep)

    path = gf.Path([
        lv2hv_ref.ports["VSS_0"].center,
        (lv2hv_ref.ports["VSS_0"].center[0], lv2hv_ref.ports["VSS_0"].center[1]-6.0)
    ])
    path_component = gf.path.extrude(
        path,
        layer = "Metal3drawing",
        width = lv2hv_ref.ports["VSS_0"].width
    )
    c.add_ref(path_component)

    path = gf.Path([
        lv2hv_ref.ports["VSS_1"].center,
        (lv2hv_ref.ports["VSS_1"].center[0], lv2hv_ref.ports["VSS_1"].center[1]-6.0)
    ])
    path_component = gf.path.extrude(
        path,
        layer = "Metal3drawing",
        width = lv2hv_ref.ports["VSS_1"].width
    )
    c.add_ref(path_component)

    path = gf.Path([
        lv2hv_ref.ports["VSS_BUFF"].center,
        (lv2hv_ref.ports["VSS_BUFF"].center[0], lv2hv_ref.ports["VSS_BUFF"].center[1]-6.0)
    ])
    path_component = gf.path.extrude(
        path,
        layer = "Metal3drawing",
    ...
    populate_via_stack(
        c, 
        column_width=0.3,
        row_width=lv2hv_ref.ports["VSS_0"].width,
        center=lv2hv_ref.ports["VSS_0"].center,
        bottom_layer="Metal2",
        top_layer="Metal3"
    )
    populate_via_stack(
        c, 
        column_width=0.3,
        row_width=lv2hv_ref.ports["VSS_1"].width,
        center=lv2hv_ref.ports["VSS_1"].center,
        bottom_layer="Metal2",
        top_layer="Metal3"
    )
    populate_via_stack(
        ...
        column_width=0.3,
        row_width=1,
        center=lv2hv_ref.ports["VSS_BUFF"].center,
        bottom_layer="Metal1",
        top_layer="Metal2"
    )
    populate_via_stack(
    ...
    path = gf.Path([
        lv2hv_ref.ports["VDD_CC"].center,
        (lv2hv_ref.ports["VDD_CC"].center[0], inverter_ref.ymax),
        (inverter_ref.ports["VDD"].center[0], inverter_ref.ymax),
        inverter_ref.ports["VDD"].center,
    ])
    path_component = gf.path.extrude(
        path,
        layer = "Metal2drawing",
    ...
    invInPoints = [
        (lv2hv_ref.ports["IN"].center[0]+1, lv2hv_ref.ports["IN"].center[1]),
        (lv2hv_ref.ports["IN"].center[0]+1, invInConny)
    ]
    ...
    invInGPoints = [
        lv2hv_ref.ports["IN"].center,
        (lv2hv_ref.ports["IN"].center[0]+1, lv2hv_ref.ports["IN"].center[1])
    ]
    path = gf.Path(invInGPoints)
    path_component = gf.path.extrude(
        path,
        layer = "Metal1drawing",
        width = invInConnWidth
    ...
    populate_via_stack(
        ...
        0.3,
        0.3,
        (lv2hv_ref.ports["IN"].center[0]+1.0, lv2hv_ref.ports["IN"].center[1])
    )

    lv2hvOutPoints = [
        lv2hv_ref.ports["OUT_P"].center,
        (lv2hv_ref.ports["OUT_P"].center[0], inverter_ref.ymax+blockSep/2),
        (inverter_ref.ports["IN"].center[0], inverter_ref.ymax+blockSep/2),
        inverter_ref.ports["IN"].center,
    ]
    path = gf.Path(lv2hvOutPoints)
    path_component = gf.path.extrude(
        path,
        layer = "Metal2drawing",
        width = invInConnWidth
    ...
    c.add_polygon(
        [
            (lv2hv_ref.ports["IN"].center[0]-invInConnExt/2, invInConny),
            (lv2hv_ref.ports["IN"].center[0]+invInConnExt/2, invInConny),
            (lv2hv_ref.ports["IN"].center[0]+invInConnExt/2, invInConny+invInConnL),
            (lv2hv_ref.ports["IN"].center[0]-invInConnExt/2, invInConny+invInConnL),
        ],
        layer="Metal2drawing"
    )
    populate_via_stack(
        ...
        center=(lv2hv_ref.ports["IN"].center[0], height-invInConnL/2),
        bottom_layer="Metal2",
        top_layer="Metal3"
    )
...
if __name__ == "__main__":
    ...
    top.flatten()
    top.write_gds(output_dir / f"gds/hm_pg_hv_{width}x{height}.gds")
